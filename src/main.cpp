...
int main() {
    ...
    //cout << output.allowed_type[firewolf::streams::logger::DEBUG] << endl;
    output.config->show_time = true;
    output.config->wait = chrono::milliseconds(200);
    output <= "1331";
    output[firewolf::streams::logger::WARNING] << "test";
    output[firewolf::streams::logger::ERROR] << "FUCK OFF";
    output[firewolf::streams::logger::DEBUG] << "FUCK OFF";
    return 0;
}
